...
- 一般模式下，非聯絡人訊息若達到高信心垃圾訊息門檻，啟用即時防護後會**直接封鎖對方**，不是只標記或只提醒；嚴格模式則會封鎖所有非聯絡人。

封鎖事件會寫入本機封鎖記錄，之後可以在 App 中查詢、產生報告或匯出。

### 文字與圖片廣告辨識

內建規則會先檢查文字；非聯絡人的**第一則訊息**不需要包含網址，只要內容達到高信心垃圾訊息門檻，就會在即時防護中直接封鎖。判斷會合併垃圾類別、導流意圖、陌生新發送者、Telegram 可疑帳號標記與刻意變形等訊號，避免「出」「彩」「賣」這類單一常用字造成誤判。使用者明確加入的學習規則則視為明確封鎖規則，命中後會直接封鎖。文字訊息與圖片 OCR 讀出的文字會進入完全相同的判斷流程。

#### 變形字、同音與學習規則

訊息會建立多個平行比對形式，不會把拼音或注音當成額外投票，因此同一個詞命中多種形式時不會無限累加分數：

- Unicode NFKC、全形／半形、大小寫、零寬字元、變體選擇器與簡繁中文正規化。
- 中文中插入空格、符號、標點或部分英數混用的變形。
- 中文原文、帶聲調拼音、無聲調拼音、帶聲調注音與無聲調注音。
- 例如「投資群組」「偷資群組」「投 資 群 組」「tou zi qun zu」與「ㄊㄡˊ ㄗ ㄑㄩㄣˊ ㄗㄨˇ」可以落到同一個規則。
- 一般兩字中文同音詞若沒有更長的詞組或其他訊號，會降低權重；但投資、賭博、色情、詐騙等高風險詞的同音形式仍會直接命中。
- 「學習規則」儲存的關鍵詞也會自動套用上述文字、拼音與注音形式；已打包的 App 不需要重新訓練模型或產生新的模型檔。

#### 預設封鎖規則

目前內建規則主要涵蓋以下類型，且每個詞組都會使用相同的中文、拼音與注音形式比對：
...
- **賭博與博彩：**博彩、線上博弈、娛樂城、賭場、推筒子、百家樂、骰寶、老虎機、球版、現金版、下注、投注、彩金、返水、賠率等。
- **詐騙與假客服：**詐騙、假客服、帳戶異常、帳戶驗證、驗證碼、解除限制、解凍帳戶、中獎、領獎、低息貸款等。
- **贈品與促銷：**註冊送、免費領、紅包、禮金、優惠碼、推廣碼等。
- **刷讚與社群操作：**點贊、關注、刷粉、刷讚、漲粉等。
- **銷售與批發：**供應、批發、代購、代發、批發價、大量現貨、一件代發等。
...
簡體中文會先轉換成繁體中文再比對，英文規則不分大小寫。這些是預設規則，不會取代白名單；若有正常對話包含上述詞彙，請將對方加入白名單。你也可以在「學習規則」中加入自己的關鍵詞與模式；自訂規則會與內建規則一起使用，且會套用相同的正規化與拼音／注音比對。
...
OCR 讀出的文字會再交給上面的預設規則與你的學習規則判斷，包含同音變形、拼音與注音比對；例如圖片中含有「頭姿」這類「投資」同音變形，命中規則後會依同樣的即時防護流程直接封鎖。OCR 不會因為圖片本身存在就封鎖，必須先讀出文字並命中規則。

macOS 版本使用本機 Vision OCR helper，不需要另外安裝 Tesseract 或把 OCR 語言模型放進 App。OCR 需要 App 中顯示「OCR 可用」才會工作。功能目前針對 Telegram 的圖片訊息；以「檔案／文件」形式傳送的圖片不一定會觸發圖片辨識，且圖片解析度、字體、背景與文字方向都可能影響結果。
...
每個 Telegram 帳號可以在「帳號詳細設定 → 防護政策」選擇模式：

- **一般模式：**只封鎖達到高信心門檻的非聯絡人，正常訊息會保留。
- **嚴格模式：**白名單與聯絡人仍然保留，其他非聯絡人不論文字內容都直接封鎖。啟用前 App 會再次確認，因為它可能封鎖正常的新對話。

黑名單優先於兩種模式；聯絡人與白名單則優先跳過一般垃圾訊息判斷。
...
| 白名單對象傳來私訊 | 跳過自動防護 |
| 黑名單對象傳來一對一私訊 | 優先直接封鎖 |
| 一般模式下非聯絡人訊息達到高信心門檻 | **直接封鎖**，並寫入封鎖記錄 |
| 嚴格模式下非聯絡人訊息 | **直接封鎖**，不等待內容判斷 |
